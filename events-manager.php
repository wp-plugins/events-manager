...
include("em-template-tags-depreciated.php"); //To depreciate
//Widgets
...
// Setting constants
define('EM_VERSION', 4.939); //self expanatory
...
class EM_Scripts_and_Styles {
	function init(){
		if( is_admin() ){
			...
			if( substr($_GET['page'],0,14) == 'events-manager' ){
	...
	function localize_script(){
		...
		$localize['event_reschedule_warning'] = __('Are you sure you want to reschedule this recurring event? If you do this, you will lose all booking information and the old recurring events will be deleted.', 'dbem');
		$localize['disable_bookings_warning'] = __('Are you sure you want to disable bookings? If you do this and save, you will lose all previous bookings. If you wish to prevent further bookings, reduce the number of spaces available to the amount of bookings you currently have', 'dbem');
		$localize['event_detach_warning'] = __('Are you sure you want to detach this event? By doing so, this event will be independent of the recurring set of events.', 'dbem');
		$delete_text = ( !EMPTY_TRASH_DAYS ) ? __('This cannot be undone.','dbem'):__('All events will be moved to trash.','dbem');
		$localize['delete_recurrence_warning'] = __('Are you sure you want to delete all recurrences of this event?', 'dbem').' '.$delete_text;
		if( is_admin() ){
			$localize['event_post_type'] = EM_POST_TYPE_EVENT;
			$localize['location_post_type'] = EM_POST_TYPE_LOCATION;
			$localize['recurrences_menu'] = get_option('dbem_cp_events_recurring_show_menu');
			$localize['locations_menu'] = get_option('dbem_cp_locations_show_menu');
		}
		wp_localize_script('events-manager','EM', $localize);
	}

	function admin_styles(){
		global $post;
		if($post->post_type == EM_POST_TYPE_EVENT || $post->post_type == 'event-recurring' || $post->post_type == EM_POST_TYPE_LOCATION || substr($_GET['page'],0,14) == 'events-manager'){
...
function em_favorite_menu($actions) {
	...
	$actions [EM_ADMIN_URL.'&amp;page=events-manager-event'] = array (__ ( 'Add an event', 'dbem' ), 'edit_events' );
	return $actions;
}
