...
function em_content($page_content) {
	...
	if( in_array($post->ID, array($events_page_id, $locations_page_id, $categories_page_id))){
...
function em_content_page_title($content) {
	...
	$events_page_id = get_option ( 'dbem_events_page' );
	$locations_page_id = get_option( 'dbem_locations_page' );
	if( !empty($post->ID) && in_array($post->ID, array($events_page_id, $locations_page_id))){
		...
		if( empty($content) ){
			...
			}elseif( EM_MS_GLOBAL && $post->ID == $locations_page_id && is_object($EM_Location) ){
...
function em_content_wp_title($title, $sep = '', $seplocation = ''){
	...
	$events_page_id = get_option ( 'dbem_events_page' );
	$locations_page_id = get_option( 'dbem_locations_page' );
	if( get_the_ID() != $events_page_id && !(EM_MS_GLOBAL && get_the_ID() == $locations_page_id && is_object($EM_Location)) ){ return $title; }
...
function em_wp_the_title($data){
	...
	$events_page_id = get_option ( 'dbem_events_page' );
	$locations_page_id = get_option( 'dbem_locations_page' );
	if( get_option('dbem_disable_title_rewrites') != 1 && !empty($post->ID) && ($post->ID == $events_page_id || (EM_MS_GLOBAL && $post->ID == $locations_page_id && is_object($EM_Location))) ){
