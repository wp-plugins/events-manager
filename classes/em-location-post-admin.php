	...
	function save_post($post_id){
		...
		if(!defined('UNTRASHING_'.$post_id) && $is_post_type && $saving_status){
			if( wp_verify_nonce($_REQUEST['_emnonce'], 'edit_location')){
				...
				$location_status = ($EM_Location->post_status == 'publish') ? 1:0;
				$wpdb->query("UPDATE ".EM_LOCATIONS_TABLE." SET location_name='{$EM_Location->location_name}', location_slug='{$EM_Location->location_slug}', location_status={$location_status} WHERE location_id='{$EM_Location->location_id}'");
				apply_filters('em_location_save', true , $this);
			}
		}
	}
