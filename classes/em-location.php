...
class EM_Location extends EM_Object {
	...
	function save(){
		...
		if( !$this->can_manage('edit_locations', 'edit_others_locations') && ( get_option('dbem_locations_anonymous_submissions') && empty($this->location_id)) ){
		...
		if( !is_wp_error($post_id) && !empty($post_id) ){
			...
			$this->load_postdata($post_data, $blog_id);
			$this->post_id = $post_id; //just in case something went wrong, just set the post_id
			//now save the meta
