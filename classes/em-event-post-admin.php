...
class EM_Event_Post_Admin{
	function init(){
		...
		//Post Customization
		if( get_option('dbem_attributes_enabled') && !get_option('dbem_cp_events_custom_fields', false) ){
			add_filter('em_cp_event_supports', array('EM_Event_Post_Admin','supported_custom_fields'),10,1);
		}
		//Save/Edit actions
			...
			if( wp_verify_nonce($_REQUEST['_emnonce'], 'edit_event') ){ 
				...
				if( !$get_meta || !$save_meta ){
					...
					if( (!$EM_Event->is_recurring() && !current_user_can('publish_events')) || ($EM_Event->is_recurring() && !current_user_can('publish_recurring_events')) ){
						if( $EM_Event->post_status == 'publish' ){ $EM_Event->set_status(0, true); } //no publishing and editing... security threat
					}else{
				...
				if( $EM_Event->validate() ){
					...
					//now update the db
					$wpdb->query("UPDATE ".EM_EVENTS_TABLE." SET event_name='{$EM_Event->event_name}', event_slug='{$EM_Event->event_slug}', event_status={$event_status} WHERE event_id='{$EM_Event->event_id}'");
					if( $EM_Event->is_recurring() &&  $EM_Event->post_status == 'publish'){
	...
	function maybe_publish_location($EM_Event){
		...
		if( $EM_Event->post_status == 'publish' && !empty($EM_Event->location_id) ){
...
if( is_admin() ) EM_Event_Post_Admin::init();
