...
class EM_Categories_Taxonomy{
	function init(){
		...
		add_action('admin_init', array('EM_Categories_Taxonomy','admin_init'));
	}
	...
	function save($term_id, $tt_id){
		...
		if( !empty($_POST['category_bgcolor']) && preg_match('/^#[a-zA-Z0-9]{6}$/', $_POST['category_bgcolor']) ){
			...
			if( count($prev_settings) > 0 ){
				$wpdb->update(EM_META_TABLE, array('object_id'=>$term_id,'meta_key'=>'category-bgcolor','meta_value'=>$_POST['category_bgcolor']), array('object_id'=>$term_id));
			}else{
		...
		if( !empty($_POST['category_image']) ){
			...
			if( count($prev_settings) > 0 ){
				$wpdb->update(EM_META_TABLE, array('object_id'=>$term_id,'meta_key'=>'category-image','meta_value'=>$_POST['category_image']), array('object_id'=>$term_id));
			}else{
