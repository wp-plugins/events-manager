...
class EM_Location_Posts_Admin{
	function init(){
		...
		if( !empty($_GET['page']) && $_GET['page'] == 'events-manager-locations' && !get_option('dbem_cp_locations_show_menu')){
			add_action('admin_init',array('EM_Location_Posts_Admin','redirect'));
		}
	}
	
	function redirect(){
		wp_redirect(admin_url().'edit.php?post_type='.EM_POST_TYPE_LOCATION); exit();
	}
