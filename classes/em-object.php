...
class EM_Object {
	...
	function build_sql_conditions( $args = array() ){
		...
		if ( is_numeric($category) && $category > 0 ){
			...
			$term = get_term($category, EM_TAXONOMY_CATEGORY);
			if( $term !== false && !is_wp_error($term) ){
				$term_id = $term->term_taxonomy_id;
				$conditions['category'] = " ".EM_EVENTS_TABLE.".post_id IN ( SELECT object_id FROM ".$wpdb->term_relationships." WHERE term_taxonomy_id=$term_id ) ";
			} 
		}elseif( self::array_is_numeric($category) ){
			$term_ids = array();
			foreach($category as $category_id){
				$term = get_term($category_id, EM_TAXONOMY_CATEGORY);
				if( $term !== false && !is_wp_error($term) ){
