...
if( !class_exists('EM_Permalinks') ){
	class EM_Permalinks {
		...
		function init(){	
			if( get_option('dbem_permalinks_legacy',true) ){
				...
				/* - depreciated, using WP permalink functions instead, all done for us!
		...
		function rewrite_rules_array($rules){
			...
			if( is_object($events_page) ){
				...
				$em_rules[$events_slug.'/'.EM_LOCATIONS_SLUG.'$'] = 'index.php?pagename='.$events_slug.'&event_locations=1'; //locations list with slug
				...
				*/
				...
				if( !get_option('dbem_cp_events_has_archive', false) ){
