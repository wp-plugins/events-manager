  	...
  	if(isset($_GET['doaction2']) && $_GET['doaction2'] == "Delete") {
	  	if(isset($_GET['action2']) && $_GET['action2'] == "delete") {
				...
				foreach($venues as $venue) {
					dbem_log("roba da cancellatre: $venue"); 
					if (false)
						dbem_delete_venue($venue);
				}
			}
		}
	}
	...
	<div class='wrap'>
		...
		<table class='form-table'>
				<tr class='form-field form-required'>
					...
					<td><input name='venue_name' id='venue_name' type='text' value='".$venue['venue_name']."' size='40' aria-required='true' /><br />
		           ".__('The name of the venue', 'dbem')."</td>
				</tr>

				<tr class='form-field'>
					<th scope='row' valign='top'><label for='venue_address'>".__('Venue address', 'dbem')."</label></th>
					<td><input name='venue_address' id='venue_address' type='text' value='".$venue['venue_address']."' size='40' /><br />
				...
				<tr class='form-field'>
					...
					<td><input name='venue_town' id='venue_town' type='text' value='".$venue['venue_town']."' size='40' /><br />
							 ...
							 <form name='addvenue' id='addvenue' method='post' action='admin.php?page=venues' class='add:the-list: validate'>\n
							    ...
							    <div class='form-field form-required'>\n
							      <label for='venue_name'>".__('Venue name', 'dbem')."</label>\n
								 	<input name='venue_name' id='venue_name' type='text' value='".$new_venue['venue_name']."' size='40' />\n
								    <p>".__('The name of the venue', 'dbem').".</p>\n
								 </div>\n
               
								 <div class='form-field'>\n
								   <label for='venue_address'>".__('Venue address', 'dbem')."</label>\n
								 	<input name='venue_address' id='venue_address' type='text' value='".$new_venue['venue_address']."' size='40'  />\n
								    <p>".__('The address of the venue', 'dbem').".</p>\n
								 </div>\n
               
								 <div class='form-field'>\n
								   <label for='venue_town'>".__('Venue town', 'dbem')."</label>\n
								 	<input name='venue_town' id='venue_town' type='text' value='".$new_venue['venue_town']."' size='40'  />\n
...
function dbem_delete_venue($venue) {
		global $wpdb;	
		$table_name = $wpdb->prefix.VENUES_TBNAME;
		$sql = "DELETE FROM $table_name WHERE venue_id = '".$venue['venue_id']."';";
	 	$wpdb->query($sql);
