...
function em_create_categories_table() {
	...
	if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name && $wpdb->get_var("SHOW TABLES LIKE '$old_table_name'") != $old_table_name) {
		dbDelta($sql);
		$term_array = wp_insert_term('Uncategorized', EM_TAXONOMY_CATEGORY, array(
			'description' => '',
			'slug' => 'uncategorized'
		));
		$wpdb->insert( $table_name, array('category_name'=>__('Uncategorized', 'dbem'), 'category_slug'=>'uncategorized', 'term_id' => $term_array['term_id']), array('%s') );
	}else{
		dbDelta($sql);
	}
	em_sort_out_table_nu_keys($table_name, array('category_slug','term_id'));
}
	...
	foreach( $categories as $category ){
		...
		if( $term === false ){
			...
			if( is_array($term_array) ){
				...
				$wpdb->query('UPDATE '.EM_CATEGORIES_TABLE." SET term_id='{$term_array['term_id']}' WHERE category_id={$category['category_id']}");
			...
			$wpdb->query('UPDATE '.EM_CATEGORIES_TABLE." SET term_id='{$term->term_id}' WHERE category_id={$category['category_id']}");
		}
