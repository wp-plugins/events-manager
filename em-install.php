...
function em_set_capabilities(){
	...
	if( get_option('dbem_version') == '' ){
		...
		$caps = array(
			/* Event Capabilities */
			'publish_events', 'delete_others_events', 'edit_others_events',
	...
	if( get_option('dbem_version') < 5 ){
		...
		$conditional_caps = array(
			...
			'edit_recurrences' => 'edit_recurring_events'
		);
	...
	if( get_option('dbem_events_page') == '' && get_option('dbem_dismiss_events_page') != 1 && !is_object( get_page( get_option('dbem_events_page') )) ){
		...
		$post_id = wp_insert_post($post_data, false);
	   	if( $post_id > 0 ){
			...
			$post_id = wp_insert_post($post_data, false);
	   		update_option('dbem_locations_page', $post_id);	
	   		//Now Categories Page
	   		$post_data = array(
				'post_status' => 'publish',
	   			'post_parent' => $post_id,
			...
			$post_id = wp_insert_post($post_data, false);
	   		update_option('dbem_categories_page', $post_id);
	   	}
	}
}   
